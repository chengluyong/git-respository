...
class DataList extends Component
{
  ...
  render()
  {
   // console.log(this.props.data);
      ...
      const _property =(v, i)=>
      {
        ...
        if(i === 80)
        {
          ...
          style={ { textAlign: 'center', 'textOverflow': 'ellipsis', 'whiteSpace': 'nowrap', 'padding': '0 0.5rem'} } key={ i }>
            <Link to={`/report/${ o.name }`} >
              <button style={ { padding: '0.15rem 0.5rem' } } className="btn btn-link  btn-sm" value={ o.name }>详情</button>
            </Link>
            </td>
        }
        return <td
    ...
    const ele = <div className={ '' } style={ {padding: '0px', margin: '0px', overflow: 'scroll', width: '100%'} }>
      <div className={ 'row' }  style={ { margin: '0px'} } >
        <div className={ 'col-12' } style={ {padding: '0px', margin: '0px'} }>

          <table className="table table-striped">
            <thead  style={ { background: "#483D8B", color: "white"} }>
              <tr>
